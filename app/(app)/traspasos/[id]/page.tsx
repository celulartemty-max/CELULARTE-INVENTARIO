...
export default async function Page({params}:{params:Promise<{id:string}>}){
  ...
  const rows=await sql`SELECT m.id::text,m.folio,m.current_version,t.id::text transfer_id,t.origin_branch_id::text,t.destination_branch_id::text,t.status::text,t.transfer_kind "transferKind",t.sent_at,t.received_at,t.origin_pos_registered_at,t.destination_pos_registered_at,o.name origin,d.name destination,(d.name IN ('Treviño','Plaza del Audio')) "destinationInternal" FROM inventory_movements m JOIN transfers t ON t.movement_id=m.id JOIN branches o ON o.id=t.origin_branch_id JOIN branches d ON d.id=t.destination_branch_id WHERE m.id=${id}::uuid`,t=rows[0]as unknown as Transfer|undefined;
  if(!t)return <p>No encontrado.</p>;
  const ids=await getAuthorizedBranchIds(u),master=ids===null,canOrigin=master||ids.includes(t.origin_branch_id),canDestination=t.destinationInternal&&(master||ids.includes(t.destination_branch_id));
