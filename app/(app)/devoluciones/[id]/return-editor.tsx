...
export default function ReturnEditor({movement,lines,evidence,products,role}:{movement:ReturnMovement;lines:ReturnLine[];evidence:Evidence[];products:ReturnProduct[];role:AppRole}){
  ...
  return <div style={{maxWidth:760,margin:'0 auto'}}>
    ...
    <section className="card pad" style={{padding:22,borderRadius:18}}>
      ...
      <div className="lines">{lines.map(l=><div className="line" key={l.id} style={{padding:14}}><div><b>{l.product}</b><small>{l.quantity} pieza{l.quantity===1?'':'s'} · {conditions[l.condition]||l.condition}</small>{l.condition==='DAMAGED'&&l.reason&&<small>{l.reason}</small>}</div>{editable&&<button onClick={()=>act({action:'deleteLine',lineId:l.id},'¿Eliminar este producto de la devolución?')}>Eliminar</button>}</div>)}</div>
      {movement.status==='DRAFT'&&<><button className="primary" style={{width:'100%',marginTop:18,padding:14,opacity:missingDamageEvidence?.5:1,cursor:missingDamageEvidence?'not-allowed':'pointer'}} disabled={lines.length===0||missingDamageEvidence} onClick={()=>act({action:'close'},`Cerrar devolución con ${lines.length} producto${lines.length===1?'':'s'} y ${total} pieza${total===1?'':'s'}?`)}>Terminar captura de devolución</button>{missingDamageEvidence&&<small style={{display:'block',marginTop:9,color:'#9a5b00',fontWeight:700,textAlign:'center'}}>Agrega una evidencia del producto dañado para continuar.</small>}</>}
    </section>

    {(hasDamaged||evidence.length>0)&&<section className="card pad" style={{padding:22,borderRadius:18,border:missingDamageEvidence?'1px solid #e6a23c':undefined}}><h2 style={{marginTop:0}}>Evidencia {hasDamaged&&<span style={{color:'#b54708'}}>*</span>}</h2><p style={{color:'#667085',marginTop:-6}}>{hasDamaged?'Obligatoria cuando hay producto dañado. Hasta 2 fotografías.':'Hasta 2 fotografías.'}</p>{[1,2].map(slot=>{const ev=evidence.find(x=>x.slot===slot);return <div className="line" key={slot}><div><b>Foto {slot}</b>{ev&&<img src={ev.fileUrl} alt={`Evidencia ${slot}`} style={{display:'block',marginTop:8,maxWidth:'100%',maxHeight:180,borderRadius:10,objectFit:'cover'}}/>}</div>{editable&&(ev?<div className="actions"><button onClick={()=>{const url=prompt('URL de la nueva evidencia');if(url)act({action:'saveEvidence',slot,fileUrl:url})}}>Reemplazar</button><button onClick={()=>act({action:'deleteEvidence',slot})}>Eliminar</button></div>:<button onClick={()=>{const url=prompt('URL de la evidencia o fotografía');if(url)act({action:'saveEvidence',slot,fileUrl:url})}}>Agregar evidencia</button>)}</div>})}</section>}
