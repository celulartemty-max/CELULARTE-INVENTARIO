...
const canEdit=(role,status)=>status==='DRAFT'||role==='MASTER';const canPos=(status,at)=>status!=='DRAFT'&&status!=='CANCELLED'&&!at;const validQty=q=>Number.isInteger(q)&&q>0;const canClose=boxes=>boxes.length>0&&boxes.every(b=>b.lines.length>0);
...
test('POS is impossible before close and only once',()=>{assert.equal(canPos('DRAFT',null),false);assert.equal(canPos('CLOSED',null),true);assert.equal(canPos('POS_REGISTERED',new Date()),false)});
