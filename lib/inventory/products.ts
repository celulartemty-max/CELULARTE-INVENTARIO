...
export async function renameColor(u:SessionUser,id:string,name:string){master(u);const n=name.trim();if(!n)throw new Error("NAME_REQUIRED");await sql`UPDATE product_colors SET name=${n},updated_at=now() WHERE id=${id}::uuid`;}
